...
public class PropertyEditEvent {
    ...
    public void editBlock() {
        if (!this.property.getPermission().has(player)) {
            this.sendNoPermPropertyMessage();
            return;
        }

        if (this.hasNotEnoughDurability()) {
            this.sendNotEnoughDurabilityMessage();
            return;
        }

        this.editBlockData();
        this.registerEditedBlock();
        LogUtil.logEdit(player, this.property, this.getBlockDataNewValue(), block, this.getBlockDataOldValue());
